...
import { addEvent } from "../eye-analysis/recorder/core"
import { getState, subscribe } from "../eye-analysis/recorder/state"
...
export const useRecorderState = () => {
  const [state, setState] = useState<RecorderState>(getState())
...
export const useGazeTracking = () => {
  const [error, setError] = useState<string | null>(null)
  const [lastGazePoint, setLastGazePoint] = useState<GazePointInput | null>(
    null,
  )
  const state = useRecorderState()
  ...
  const stopAndDownloadSession = useCallback(
    async (experimentMetadata?: Record<string, unknown>) => {
      ...
      if (recording.currentSession) {
        await sessionExport.saveExperiment(
