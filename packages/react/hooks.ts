...
export const useGazeTracking = () => {
  ...
  const addUserEvent = useCallback(async (type: string, data?: Record<string, any>) => {
    try {
      setError(null);
...
export const useSessionExport = () => {
  ...
  const downloadSession = useCallback(async (
    ...
    try {
      ...
      await downloadSessionAsZip(targetSessionId, options);
    } catch (err) {
  ...
  const saveAndDownloadExperiment = useCallback(async (
    sessionId?: string,
    ...
    try {
      ...
      await saveExperimentData(targetSessionId, experimentMetadata);
    } catch (err) {
...
export const useEyeTracker = (autoInitialize: boolean = true) => {
  ...
  }, [autoInitialize, recording.isInitialized]);
  ...
  const stopAndDownloadSession = useCallback(async (
    experimentMetadata?: Record<string, any>
  ) => {
