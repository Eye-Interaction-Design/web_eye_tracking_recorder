...
export class ExperimentDatabase {
  ...
  private createStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains('gazeData')) {
      const gazeStore = db.createObjectStore('gazeData', { keyPath: 'id', autoIncrement: true });
    ...
    if (!db.objectStoreNames.contains('syncMarkers')) {
      const syncStore = db.createObjectStore('syncMarkers', { keyPath: 'id', autoIncrement: true });
  ...
  async getSessionData(sessionId: string): Promise<{
    ...
    const [session, videoChunks, gazeData, events, syncMarkers] = await Promise.all([
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    const transaction = this.db.transaction(['sessions', 'videoChunks', 'gazeData', 'events', 'syncMarkers'], 'readwrite');
  ...
  private deleteFromStore(transaction: IDBTransaction, storeName: string, key: string): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  private deleteFromIndex(transaction: IDBTransaction, storeName: string, indexName: string, key: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const store = transaction.objectStore(storeName);
